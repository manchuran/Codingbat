...
def last2(str):
    ...
    '''
    return [''.join(str[i:i+2]) for i in range(len(str)-2)].count(str[-2:])
...
def string_match(a, b):
    ...
    '''
    return len([value for value in 
                    [(a[i:i+2],i) for i in range(len(a)-1)]
                        if value in 
                            [(b[i:i+2],i) for i in range(len(b)-1)]
                ])
